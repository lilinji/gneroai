...
source ~/miniforge3/envs/ml_enterprise/bin/activate
...
case $TASK_TYPE in
    "development")
        echo "初始化开发环境..."
        source ~/miniforge3/envs/dev/bin/activate 2>/dev/null || echo "开发环境不存在"
        echo "可用工具: vim, git, jupyter, vscode"
        ;;
    "training")
        echo "初始化训练环境..."
        source ~/miniforge3/envs/training/bin/activate 2>/dev/null || echo "训练环境不存在"
        export CUDA_VISIBLE_DEVICES=0,1
        echo "训练环境就绪，可使用多GPU并行训练"
        ;;
    "inference")
        echo "初始化推理环境..."
        source ~/miniforge3/envs/inference/bin/activate 2>/dev/null || echo "推理环境不存在"
...
source ~/miniforge3/envs/monitoring/bin/activate 2>/dev/null || python3 -m venv monitoring_env && source monitoring_env/bin/activate
