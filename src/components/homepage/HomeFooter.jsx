...
const products = [
  ...
  //   href: '/guides/runtime-environment/overview',
  // },
  // {
  //   name: 'Live Streaming',
...
function Status({ className }) {
  ...
  return (
    <Link
      href="https://bmdc.biodesign.ac.cn/"
      className={clsx(
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3">
        {links.map(({ name, href, isAccordion, content }, index) => (
          <Link
            key={`${href}-${index}`}
            href={href}
    ...
    <footer className="bg-[#F4F7FF] dark:bg-[#191919]">
      <div className="mx-auto flex w-full max-w-[1080px] flex-col px-6 py-12">
        ...
        {/* <div className="grid grid-cols-2 gap-6 gap-y-12 md:justify-between lg:flex lg:flex-wrap">
          <Links key="product" name="Product" links={products} />
          <Links key="developers" name="Developers" links={developers} />
          <Links key="usecases" name="Usecases" links={usecases} />
          <Links key="company" name="Company" links={company} />
          <Links key="compare" name="Compare" links={comparisons} />
        </div> */}
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between lg:gap-0">
          ...
          <div className="flex flex-wrap gap-2 text-sm text-gray-500">
            <Link
              href="https://www.biodesign.ac.cn/"
            ...
            <span className="text-inherit dark:text-[#999]">
              &copy; {new Date().getFullYear()} BDC Inc.
            </span>
          </div>

          <div className="flex items-center gap-4">
            <Link
              href="https://github.com/tib-hpc"
              aria-label="GitHub Organization"
              className="transition-transform hover:scale-110"
            >
              <Github className="h-7 w-7 text-zinc-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors" />
            </Link>
          </div>
        </div>
      </div>
